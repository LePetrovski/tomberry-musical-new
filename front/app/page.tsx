...
export default async function HomePage() {
  const [podcasts, posts] = await Promise.all([
    client.fetch<PodcastPreview[]>(latestPodcastsQuery).catch(() => []),
    client.fetch<PostPreview[]>(latestPostsQuery).catch(() => []),
  ]);
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        <div className="mb-10 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-zinc-600">Les podcasts les plus récents.</p>
          </div>
          <Link href="/podcasts" className="text-sm font-medium text-zinc-700 hover:text-zinc-900">
            Voir tout →
          </Link>
        </div>
        {podcasts.length > 0 ? (
          ...
          <p className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-8 text-zinc-600">
            Aucun podcast publié pour le moment. Ajoutez du contenu dans Sanity Studio.
          </p>
        )}
      </section>
